    ...
    var active = {
      add: function(){
        var index = layer.open({
          type: 2
          ,title: '添加'
          ,content: '${ctx}/Paper/addAndUpdate.do'
          ,maxmin: true
          ,area: ['800px', '600px']
          ,btn: ['确定', '取消']
    ...
    table.on("tool(test-table-reload)", function(e) {
		...
		if ("edit" === e.event) { 
			var topicEdit = layer.open({
				type : 2,
				title : "编辑",
				content : "${ctx}/Paper/addAndUpdate.do?id="+e.data.id
				,maxmin: true
		          ,area: ['50%', '100%']
		          ,btn: ['提交','取消']
		          ,yes: function(index, layero){
			...
			layer.confirm('确定删除吗？', function(index) {
	            ...
	            $.ajax({
	                type: "POST",//方法类型
	                dataType: "json",//预期服务器返回的数据类型
	                url: "${ctx}/Paper/del.do?id="+e.data.id,//url
